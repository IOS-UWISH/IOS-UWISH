...
class TaskReportViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cellId = "TaskReportCell"
        let mission = missions[indexPath.row]
        /* 將cell轉型成BookCell */
        let cell = tableView.dequeueReusableCell(withIdentifier: cellId) as! TaskReportTableViewCell
        ...
        cell.lbTitle.text = mission.title
    ...
    @objc func show(sender:UIButton) {
        ...
        print("reportId: \(reportId), userId: \(userId)")
        ...
        let ignore = UIAlertAction(title: "忽略", style: .default) { (alertAction) in
            print("忽略有被觸發到！")
            self.updateReportStatus(reportId,userId,1,true)
        }
        let OffShelf = UIAlertAction(title: "下架", style: .default) { (alertAction) in
            print("下架有被觸發到！")
            self.updateReportStatus(reportId,userId,2,true)
        }
        let Suspension = UIAlertAction(title: "停權", style: .destructive) { (alertAction) in
            print("停權有被觸發到！")
            self.updateReportStatus(reportId,userId,3,true)
        }
    ...
    //取得被檢舉的任務：
    func showMissions() {
        let requestParam = ["action" : "getMissionReport"]
        executeTask(url_server!, requestParam) { (data, response, error) in
            if error == nil{
                if data != nil{
                    // 將輸入資料列印出來除錯用
