...
CREATE TABLE `TaskReport` (
  ...
  `Status` int DEFAULT '0' COMMENT '審核狀態（0：未處理、1：忽略、2：下架、3：停權）',
  PRIMARY KEY (`UUID`),
  KEY `fk_task_report_idx` (`Task_id`),
  KEY `fk_user_report_idx` (`Reporter_id`),
  CONSTRAINT `fk_task_report` FOREIGN KEY (`Task_id`) REFERENCES `TaskInfo` (`UUID`),
  CONSTRAINT `fk_user_report` FOREIGN KEY (`Reporter_id`) REFERENCES `UserInfo` (`UUID`)
